...
app.post("/lock/:id", (req, res) => {
  ...
  if (seat.state === "booked") return res.status(400).json({ message: `Seat ${id} is already booked.` });
  if (seat.state === "locked") return res.status(400).json({ message: `Seat ${id} is already locked.` });
...
app.post("/confirm/:id", (req, res) => {
  ...
  if (!seat) return res.status(404).json({ message: `Seat ${id} does not exist.` });
  if (seat.state !== "locked") return res.status(400).json({ message: "Seat is not locked and cannot be booked" });
